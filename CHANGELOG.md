...
- 修复 Docker standalone 部署配置
- 修复 Prisma binaryTargets Docker 兼容性问题
- 清理提交到 Git 的 .env 文件（安全修复）

#### ⚡ 改进

- 添加 `output: 'standalone'` 到 next.config.js
- 更新 Dockerfile 正确复制静态文件和 public 文件夹
- docker-compose.yml 使用环境变量替代硬编码密钥
- 添加 `.env.docker.example` 用于本地 Docker 开发
